    ...
    actions: {
        ...
        async update({ _ }, params) {
            try {
                ...
                formData.append('paypal_tokenization_key', params.paypal_tokenization_key);
                ...
                formData.append('php_mail_email_from_name', params.php_mail_email_from_name);
                ...
                formData.append('display_facebook_pixel', params.display_facebook_pixel);
                formData.append('google_rechaptcha_key', params.google_rechaptcha_key);
                formData.append('google_rechaptcha_secret', params.google_rechaptcha_secret);
                formData.append('cookies_agreement', params.cookies_agreement);

                const response = await axios.post(`/settings/${params.id}`, formData);
                Swal.fire({
                    position: "top-end",
                    icon: "success",
                    title: "Is Updated",
        ...
        async destroy({ _ }, id) {
            try {
                const result = await Swal.fire({
                    title: "Are you sure?",
                    text: "You won't be able to revert params!",
                    icon: "warning",
